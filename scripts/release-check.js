...
const failures=[];
if(pkg.version!=='2.3.0') failures.push(`package version is ${pkg.version}`);
...
if(lock.version!=='2.3.0' || lock.packages?.['']?.version!=='2.3.0') failures.push('package-lock version mismatch');
...
const expected={version:'2.3.0',files:manifest};
...
console.log(`AnchorWeight v2.3.0 release ${VERIFY?'verification':'check'} PASS (${manifest.length} source files manifested)`);
