...
if(lock.version!=='1.3.0' || lock.packages?.['']?.version!=='1.3.0') failures.push('package-lock version mismatch');
...
fs.writeFileSync(path.join(root,'SOURCE-MANIFEST.json'),JSON.stringify({version:'1.3.0',files:manifest},null,2));

if(failures.length){for(const f of failures)console.error('FAIL:',f);process.exit(1)}
console.log(`AnchorWeight v1.3.0 release check PASS (${manifest.length} source files manifested)`);
