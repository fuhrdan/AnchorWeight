...
const required=['README.md','SECURITY.md','ARCHITECTURE.md','THREAT-MODEL.md','CONFIGURATION.md','OPERATIONS.md','DEPLOYMENT.md','CLI.md','LICENSE','DISTRIBUTED-INTELLIGENCE.md','OPERATOR-GATEWAY.md','public/setup.html','src/setup.js','src/telemetry.js','hub.js'];
const failures=[];
if(pkg.version!=='1.7.0') failures.push(`package version is ${pkg.version}`);
...
if(lock.version!=='1.7.0' || lock.packages?.['']?.version!=='1.7.0') failures.push('package-lock version mismatch');
...
fs.writeFileSync(path.join(root,'SOURCE-MANIFEST.json'),JSON.stringify({version:'1.7.0',files:manifest},null,2));

if(failures.length){for(const f of failures)console.error('FAIL:',f);process.exit(1)}
console.log(`AnchorWeight v1.7.0 release check PASS (${manifest.length} source files manifested)`);
