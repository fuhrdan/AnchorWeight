  ...
  function handle(req, res, url) {
    ...
    if (url.pathname.startsWith(`${config.basePath}/api/`)) {
      ...
      if (url.pathname === `${config.basePath}/api/session`) {
        ...
        return json(res, 200, { version:'1.1.0', csrfToken, expiresInSeconds:(config.csrfTtlMinutes || 15) * 60 });
      }

      if (url.pathname === `${config.basePath}/api/stats` || url.pathname === `${config.basePath}/api/stats/`) {
        return json(res, 200, { version: '1.1.0', mode: config.shadowMode ? 'shadow' : 'enforce', blockDepth: config.blockDepth, scoreEnforcementEnabled: !!config.scoreEnforcementEnabled, quarantineScore: config.quarantineScore ?? 100, ...store.snapshot() });
      }
      ...
      if (url.pathname === `${config.basePath}/api/events`) {
        ...
        return json(res, 200, { version:'1.1.0', events });
      }
