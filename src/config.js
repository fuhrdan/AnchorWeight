...
export function loadConfig(overrides = {}) {
  ...
  return {
    ...
    // v2.3: circuit breakers and fallback destinations are opt-in and process-local.
    resilienceEnabled: boolEnv('AW_RESILIENCE_ENABLED', false),
    resilienceFallbackFile: process.env.AW_RESILIENCE_FALLBACKS_FILE || '',
    resilienceAllowedOrigins: listEnv('AW_RESILIENCE_ALLOWED_ORIGINS'),
    resilienceFailureThreshold: intEnv('AW_RESILIENCE_FAILURE_THRESHOLD', 3, 2, 20),
    resilienceCooldownMs: intEnv('AW_RESILIENCE_COOLDOWN_MS', 30000, 1000, 300000),
    resilienceHealthEnabled: boolEnv('AW_RESILIENCE_HEALTH_ENABLED', false),
    resilienceHealthIntervalMs: intEnv('AW_RESILIENCE_HEALTH_INTERVAL_MS', 30000, 5000, 300000),
    resilienceHealthTimeoutMs: intEnv('AW_RESILIENCE_HEALTH_TIMEOUT_MS', 2000, 500, 10000),
