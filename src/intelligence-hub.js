...
export function createIntelligenceHub({sites, file, adminToken, now = () => Date.now()} = {}) {
  ...
  const server = http.createServer((req,res) => {
    ...
    if (pathname === '/live' && req.method === 'GET') return response(res,200,{ok:true,service:'AnchorWeight Intelligence Hub',version:'2.0.0'});
    if (pathname === '/v1/events' && req.method === 'GET') {
      ...
      return response(res,200,{version:'2.0.0',siteCount:new Set(events.map(e=>e.siteId)).size,
