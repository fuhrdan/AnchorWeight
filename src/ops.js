...
export async function backupFiles(config, destination) {
  ...
  const manifest={version:'1.6.0',stateBackend:backend,createdAt:new Date().toISOString(),profile:config.profileName||null,copied};
...
export function migrateStateFile(file, options={}) {
  ...
  fs.writeFileSync(temp,JSON.stringify({
    ...migrated.state,
    meta:{...(migrated.state.meta||{}),appVersion:'1.6.0',migratedBy:'anchorweight migrate',writtenAt:new Date().toISOString()}
  }));
